...
# HillSequencer — 8-track sample sequencer.
...
if [ -n "$controlfolder" ] && [ -f "$controlfolder/control.txt" ]; then
  ...
  echo "[HillSequencer] PortMaster control folder not found; running standalone."
  GAMEDIR="$SCRIPTDIR/hillsequencer"
fi
...
# Samples are SHARED with HillChord (device Samples folder). HillSequencer's
# render cache lives separately (.hillsequencer_cache) so it isn't browsed.
